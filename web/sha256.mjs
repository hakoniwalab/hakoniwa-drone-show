...
function bytes(value) {
  ...
  throw new TypeError('SHA-256 input must be binary data');
}
...
export function sha256Fallback(value) {
  ...
  for (let index = 0; index < hash.length; index += 1) {
    resultView.setUint32(index * 4, hash[index]);
  }
  return result;
}

export async function sha256Bytes(value, cryptoObject = globalThis.crypto) {
  const input = bytes(value);
