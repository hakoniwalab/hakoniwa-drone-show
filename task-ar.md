...
カメラ重畳ARの技術検証は完了したが、スマートフォンでは初回表示とVisual Stateの
高頻度通信が重く、一般客向け体験として仕上げるには専用の軽量再生方式が必要になる。
この最適化は独立した商品開発として扱い、PLATEAU AWARD向け実装とデモの対象からは
AR対応を外す。既存AR実装と検証記録は削除せず、将来再開できる状態で保留する。

PLATEAU AWARDでは、PLATEAUの実在都市モデルと箱庭/MuJoCoの物理シミュレーションを
直接活かせる、City版の風・突風によるドローン挙動の検証を優先する。
...
- [x] Formationを観客側へ緩く湾曲させ、Show IRへ実3D座標として格納する
- [x] 近距離では立体LEDコアと機体を見せ、遠距離ではLEDハローを強調する
...
## 将来拡張: Show IRローカル再生と時刻同期

初期PoCおよびPLATEAU AWARDには含めない。一般客へ多数配信する段階では、全機体の
Visual Stateを高頻度配信しない。Show IRを初回配信し、Runnerから現在フェーズ、箱庭の
仮想時刻、再生状態だけを低頻度で通知する。ブラウザは同じShow IRを使って各機の位置、
姿勢、LEDをローカル再生する。

```text
初回: Show IR + hash
定期: run_id + phase/frame + hakoniwa_sim_time + state
操作: START / PAUSE / SEEK
位置: ブラウザ内でShow IRを評価して補間
姿勢: quaternionをslerp
LED: Show IRと仮想時刻から再生
```

- [ ] Show Statusへ箱庭仮想時刻を載せる互換性のある形式を設計する
- [ ] 現在フェーズと仮想時刻からShow IR再生位置を一意に復元できるようにする
- [ ] 2 Hz程度の同期通知で許容できる時刻誤差を定義する
- [ ] 通知間を端末時計で進め、補正時に位置やLEDを急変させない
- [ ] 位置は補間し、姿勢はslerpし、通信断時の外挿時間に上限を設ける
- [ ] CDN配信したShow IRとruntimeのhash一致を検証する
- [ ] 再接続、バックグラウンド復帰、途中参加の同期規則を定義する
- [ ] 同時接続数とサーバー帯域を測定する
- [ ] 距離別LODとして、近距離は機体とLED、中距離は簡略機体、遠距離はLEDのみを描画する
- [ ] 遠距離ではプロペラ回転を停止し、スマートフォンのCPU/GPU負荷を抑える
