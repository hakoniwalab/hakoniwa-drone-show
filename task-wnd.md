...
物理入力の正本は`enabled`と`vector_ros_m_s`だけとする。風速と風向はブラウザがベクトルから
算出して表示し、通信JSONへ重複して持たせない。風向表示は気象表現の「その方角から吹く」を
使い、風が進む向きと混同しない。受信側は有限値、範囲、schema、sequenceを検証してから
適用する。
...
全Drone分のDisturbanceオブジェクトまたは送信用バッファを前もって生成する。更新時は風ベクトル
に対応する値だけを変更し、対象全機へ1回ずつflushする。
